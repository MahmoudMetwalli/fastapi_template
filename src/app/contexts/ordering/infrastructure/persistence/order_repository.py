...
`catalog`-specific. Explicitly subclasses `OrderRepository` too, same
reasoning as `catalog`'s `book_repository.py`.
"""
...
class SqlAlchemyOrderRepository(SqlAlchemySessionScoped, OrderRepository):
